...
In the final table "catalog_calc_all_metrics", we use 'Amazon Buy Box' as the chosen competitor and '2019-12-05' as the chosen index date so that the number of rows in all tables match
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`
OPTIONS(
...
-- Step 2: Catalog calculator tables

-- Step 2.1: Non-spreadbased metrics (GRS, total order count, total ordered products, OrderShare, Cart Quantity, AOV, ACO, and AUP)

DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_non_sb_metrics`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_non_sb_metrics`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
SELECT 
  ...
  ROUND(now.GRS / now.TotalOrderCount, 2) AS Average_Contribution_to_Order,
  ROUND(now.GRS / now.OrderShare, 2) AS Average_Order_Value,
  ROUND(now.GRS / now.CartQuantity, 2) AS Average_Unit_Price,
  ...
  ROUND(ly.GRS_LY / ly.TotalOrderCount_LY, 2) AS Average_Contribution_to_Order_LY,
  ROUND(ly.GRS_LY / ly.OrderShare_LY, 2) AS Average_Order_Value_LY,
  ROUND(ly.GRS_LY / ly.CartQuantity_LY, 2) AS Average_Unit_Price_LY
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon` day
LEFT JOIN ( -- This sub-query calculates the daily GRS, total order count, total ordered products, OrderShare, and Cart Quantity over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
    AND CAST(rt.orderdate AS DATE) IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates this year
  ...
  GROUP BY 1,2
) now ON day.Datum = now.orderdate AND day.SoID = now.SoID
...
LEFT JOIN ( -- This sub-query calculates the daily GRS, total order count, total ordered products, OrderShare, and Cart Quantity over the same time period as above, BUT LAST YEAR
  ...
  WHERE 1=1
    ...
    AND CAST(rt.orderdate AS DATE) IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates last year
    AND rt.soid IN (321,368) -- 321 for UK, 368 for DE
    AND businesssector = 0 -- Direct online
    AND retailor.orcancelled != 1 -- Exclude cancellations
    AND d.orosid != 17 -- Exclude External Order Source
  GROUP BY 1,2
) ly ON day.Datum_LY = ly.orderdate_LY AND day.SoID = ly.SoID
ORDER BY 1,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_sb_metrics`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_sb_metrics`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppm_atordergm`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppm_atordergm`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
LEFT JOIN ( -- This sub-query calculates the daily PPM and AtOrderGM over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
      ...
      AND DATE(orca.orcompletedate) IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates this year
  ...
  GROUP BY 1,2
) now ON day.Datum = now.Datum AND day.SoID = now.SoID
...
LEFT JOIN ( -- This sub-query calculates the daily PPM and AtOrderGM over the same time period as above, BUT LAST YEAR
  ...
  WHERE 1=1
      AND orca.soid IN (321,368) -- 321 for UK, 368 for DE
      AND DATE(orca.orcompletedate) IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates this year
      AND orca.TotalLiquidationDiscountRevenuePreTax = 0 -- Removes liquidation orders (liquidations are one-off warehouse events and will lead to extreme adjustments; also unfair to distribute across short 3-month timeframe)
      AND orca.ProductCostDTP > 0 -- Ignore orders with $0 original costs (zero-cost tile/fabric samples and other odd items that should not impact pricing)
      AND orca.ProductRevenueDTP > 0 
      AND orca.CancelledOriginalOpQty = 0 -- Excludes cancelled orders (cancellations do not incur cost for Wayfair)
      ...
      AND ABS(orca.ProductCostDTP / orca.OriginalOpQty - 9999) >= .005 -- Ignore default value cases where ProductCostDTP is a multiple of 9999
  GROUP BY 1,2
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.SoID
ORDER BY 1,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_visits`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_visits`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
LEFT JOIN ( -- This sub-query calculates the daily visits and page views over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
    ...
    AND v.SessionStartDate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates this year
  ...
  GROUP BY 1,2
) now ON day.Datum = now.SessionStartDate AND day.SoID = now.Event_SoID
...
LEFT JOIN ( -- This sub-query calculates the daily visits and page views over the same time period as above, BUT LAST YEAR
  ...
  WHERE 1=1
    AND v.Event_SoID IN (321,368) -- 321 for UK, 368 for DE
    AND v.SessionStartDate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`) -- The relevant list of dates last year
    AND ex.VisitExclusionreason = 0 -- That's a standard filter to get the right number of visits and views
	 -- (EDIT on 20210702 - commented out the condition associated with the table aliased by "pr")
    --AND pr.Event_PrSKU IS NOT NULL -- Eliminate sessions where the SKU visited was not on record (Uncomment if you want to pull the visits that have at least 1 SKU view. Commenting this part out pulls the total traffic coming to the website)
  GROUP BY 1,2
) ly ON day.Datum_LY = ly.SessionStartDate_LY AND day.SoID = ly.Event_SoID;

-------------------------------------------------------------------------------------------------------------------------------------------------------------------

-- Step 2.5: Revenue per visit (RPV), gross profit per visit (PPV), and conversion rate (CR)

DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppv_rpv_cr`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppv_rpv_cr`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_non_sb_metrics` sb
INNER JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_sb_metrics` nonsb ON sb.SoID = nonsb.SoID AND sb.Datum = nonsb.Datum AND sb.Datum_LY = nonsb.Datum_LY
INNER JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_visits` vis ON sb.SoID = vis.SoID AND sb.Datum = vis.Datum AND sb.Datum_LY = vis.Datum_LY;

-------------------------------------------------------------------------------------------------------------------------------------------------------------------

-- Step 2.6: Indices (RPI, WPI, WSI)

DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_rpi`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_rpi`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
LEFT JOIN ( -- This sub-query calculates the daily RPI over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1  -- (EDIT on 20210702 - replaced real with kaufland and added home24 and reuter to the DE competitor list)
    AND rpi.insertdate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  ...
  GROUP BY 1,2,3
) now ON day.Datum = now.Datum AND day.SoID = now.SoID AND comp.CompetitorName = now.CompetitorName
...
LEFT JOIN ( -- This sub-query calculates the daily RPI over the same time period as above, BUT LAST YEAR
  ...
  INNER JOIN `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_dim_pc_opc_competitor` com ON com.competitorid = rpi.competitorID
  WHERE 1=1  -- (EDIT on 20210702 - replaced real with kaufland and added home24 and reuter to the DE competitor list)
    AND rpi.insertdate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
    AND CompetitorName IN ('Amazon Buy Box', 'ebayuk', 'argos', 'JohnLewis', 'kaufland', 'otto', 'heatandplumb', 'home24', 'reuter') -- Tier one competitors in the UK and DE (heatandplumb is only relevant for UK plumbing)
    AND rpi.soid IN (321,368)
  GROUP BY 1,2,3
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.SoID AND comp.CompetitorName = ly.CompetitorName
-- (EDIT on 20210702 - replaced real with kaufland and added home24 and reuter to the DE competitor list)
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wpi`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wpi`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
LEFT JOIN ( -- This sub-query calculates the daily WPI over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
    AND wpi.insertdate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  ...
  GROUP BY 1,2,3
  ORDER BY 3,2,1
) now ON day.Datum = now.Datum AND day.SoID = now.SoID AND ind.indexdate = now.IndexDate
...
LEFT JOIN ( -- This sub-query calculates the daily WPI over the same time period as above, BUT LAST YEAR
  ...
  FROM `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_fact_pc_opc_wpi_sku` wpi
  WHERE 1=1
    AND wpi.insertdate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
    AND wpi.indexdate IN (SELECT DISTINCT indexdate FROM `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_fact_pc_opc_wpi_sku` WHERE indexdate >= DATE('2019-12-05')) -- All index dates >= '2019-12-05'
    AND wpi.soid IN (321,368)
  GROUP BY 1,2,3
  ORDER BY 3,2,1
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.SoID AND ind.indexdate = ly.IndexDate_LY
ORDER BY 1,7,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wsi`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wsi`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon` day
-- Cross join ALL index dates >= '2019-12-05' to every SoID-Datum-WeekNum-Day-Month-Datum_LY row. This essentially creates a n-row partition out of every row where n is the number of index dates in the SELECT statement below
CROSS JOIN (SELECT DISTINCT indexdate FROM `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_fact_pc_opc_wsi_sku` WHERE indexdate >= DATE('2019-12-05') ORDER BY 1 DESC) ind
...
LEFT JOIN ( -- This sub-query calculates the daily WSI over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
    AND wsi.insertdate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
...
LEFT JOIN ( -- This sub-query calculates the daily WSI over the same time period as above, BUT LAST YEAR
  ...
  FROM `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_fact_pc_opc_wsi_sku` wsi
  WHERE 1=1
    AND wsi.insertdate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
    AND wsi.indexdate IN (SELECT DISTINCT indexdate FROM `wf-gcp-us-ae-pricing-prod.pricing_dw.tbl_fact_pc_opc_wsi_sku` WHERE indexdate >= DATE('2019-12-05')) -- All index dates >= '2019-12-05'
    AND wsi.soid IN (321,368)
  GROUP BY 1,2,3
  ORDER BY 3,2,1
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_availability`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_availability`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon` day 
LEFT JOIN ( -- This sub-query calculates the availability metrics over the last 3 completed months + current MTD + another 30 days
  SELECT
  ...
  WHERE 1=1
    ...
    AND PARSE_DATE('%Y%m%d', CAST(retaildate AS STRING)) IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  GROUP BY 1,2
) now ON day.Datum = now.Datum AND day.SoID = now.SoID

LEFT JOIN ( -- This sub-query calculates the availability metrics over the same time period but LAST YEAR
  SELECT
  ...
  WHERE 1=1
    AND sty_id IN (2,3) -- 2 for the UK; 3 for DE
    AND PARSE_DATE('%Y%m%d', CAST(retaildate AS STRING)) IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  GROUP BY 1,2
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.SoID
ORDER BY 1,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ttd`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ttd`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon` day 
LEFT JOIN ( -- This sub-query calculates the TTD estimate over the last 3 completed months + current MTD + another 30 days
  SELECT
    DISTINCT v.SessionStartDate AS Datum,
    v.Event_SoID,
    PERCENTILE_CONT(DATE_DIFF(CAST(p.DeliveryEstimate AS DATE), v.SessionStartDate, DAY), 0.5) OVER(PARTITION BY v.SessionStartDate,v.Event_SoID) AS Time_to_Delivery_Est_Days -- EDIT ON 12072021 changed the aggregation from avg to median
    --ROUND(AVG(DATE_DIFF(CAST(p.DeliveryEstimate AS DATE), v.SessionStartDate, DAY)), 1) AS Time_to_Delivery_Est_Days -- Calculate the average daily TTD estimate on catalog level -- EDIT ON 12072021 changed the aggregation from avg to median
	FROM `wf-gcp-us-ae-sf-prod.curated_clickstream.tbl_dash_visits` v 
  ...
  WHERE 1=1
    ...
    AND v.SessionStartDate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
    AND ex.VisitExclusionreason = 0 -- That's a standard filter to get the right number of visits and views
  ...
  --GROUP BY 1,2  -- EDIT ON 12072021 commenting because the aggregation was changed from avg to median
) now ON day.Datum = now.Datum AND day.SoID = now.Event_SoID

LEFT JOIN ( -- This sub-query calculates the TTD estimate over the same time horizon but last year
  SELECT
    DISTINCT v.SessionStartDate AS Datum_LY,
    v.Event_SoID,
    PERCENTILE_CONT(DATE_DIFF(CAST(p.DeliveryEstimate AS DATE), v.SessionStartDate, DAY), 0.5) OVER(PARTITION BY v.SessionStartDate,v.Event_SoID) AS Time_to_Delivery_Est_Days_LY -- EDIT ON 12072021 changed the aggregation from avg to median
    --ROUND(AVG(DATE_DIFF(CAST(p.DeliveryEstimate AS DATE), v.SessionStartDate, DAY)), 1) AS Time_to_Delivery_Est_Days_LY -- Calculate the average daily TTD estimate on catalog level -- EDIT ON 12072021 changed the aggregation from avg to median
    FROM `wf-gcp-us-ae-sf-prod.curated_clickstream.tbl_dash_visits` v 
  INNER JOIN `wf-gcp-us-ae-sf-prod.curated_clickstream.tbl_dash_clicks_product_request` p
    ON 1=1 
      AND p.SessionStartDate = v.SessionStartDate
  ...
  WHERE 1=1
    AND v.Event_SoID IN (321,368) -- 321 for the UK, 368 for DE
    AND v.SessionStartDate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
    AND ex.VisitExclusionreason = 0 -- That's a standard filter to get the right number of visits and views
    AND p.Event_PrSKU IS NOT NULL -- That's a standard filter to get the right number of visits and views
    AND p.DeliveryEstimate > p.SessionStartDate -- Eliminate records where the delivery estimate is less than the session start date (a potential error)
  --GROUP BY 1,2  -- EDIT ON 12072021 commenting because the aggregation was changed from avg to median
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.Event_SoID
ORDER BY 1,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ad_tag_coverage`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ad_tag_coverage`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
LEFT JOIN ( -- This sub-query calculates the attribute driving tag coverage over the last 3 completed months + current MTD + another 30 days
  ...
  WHERE 1=1
    ...
    AND snapshotdate IN (SELECT Datum FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  GROUP BY 1,2
) now ON day.Datum = now.Datum AND day.SoID = now.SoID
...
LEFT JOIN (
  ...
  WHERE 1=1
    AND bclgid IN (2,3) -- 2 for UK, 3 for DE
    AND snapshotdate IN (SELECT Datum_LY FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_time_horizon`)
  GROUP BY 1,2
) ly ON day.Datum_LY = ly.Datum_LY AND day.SoID = ly.SoID
ORDER BY 1,2;
...
DROP TABLE IF EXISTS `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_all_metrics`;
CREATE TABLE `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_all_metrics`
OPTIONS(
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 30 DAY)
) AS
...
FROM `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_non_sb_metrics` nonsb
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_sb_metrics` sb ON nonsb.Datum = sb.Datum AND nonsb.SoID = sb.SoID AND nonsb.Datum_LY = sb.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppm_atordergm` gm ON nonsb.Datum = gm.Datum AND nonsb.SoID = gm.SoID AND nonsb.Datum_LY = gm.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_visits` vis ON nonsb.Datum = vis.Datum AND nonsb.SoID = vis.SoID AND nonsb.Datum_LY = vis.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ppv_rpv_cr` pv ON nonsb.Datum = pv.Datum AND nonsb.SoID = pv.SoID AND nonsb.Datum_LY = pv.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_rpi` rpi ON nonsb.Datum = rpi.Datum AND nonsb.SoID = rpi.SoID AND nonsb.Datum_LY = rpi.Datum_LY AND rpi.CompetitorName = 'Amazon Buy Box' -- Need this condition so that the number of rows match the other tables
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wpi` wpi ON nonsb.Datum = wpi.Datum AND nonsb.SoID = wpi.SoID AND nonsb.Datum_LY = wpi.Datum_LY AND wpi.IndexDate = DATE('2019-12-05') -- Need this condition so that the number of rows match the other tables
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_wsi` wsi ON nonsb.Datum = wsi.Datum AND nonsb.SoID = wsi.SoID AND nonsb.Datum_LY = wsi.Datum_LY AND wsi.IndexDate = '2019-12-05' -- Need this condition so that the number of rows match the other tables
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_availability` av ON nonsb.Datum = av.Datum AND nonsb.SoID = av.SoID AND nonsb.Datum_LY = av.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ttd` ttd ON nonsb.Datum = ttd.Datum AND nonsb.SoID = ttd.SoID AND nonsb.Datum_LY = ttd.Datum_LY
LEFT JOIN `wf-gcp-us-ae-pricing-prod.eu_profit_mgmt.catalog_calc_ad_tag_coverage` tag ON nonsb.Datum = tag.Datum AND nonsb.SoID = tag.SoID AND nonsb.Datum_LY = tag.Datum_LY
ORDER BY 2,1;
